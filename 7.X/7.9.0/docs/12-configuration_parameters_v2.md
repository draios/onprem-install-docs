...
Parameters for the `sysdig-common-config` chart (shared configuration and behavior across the installer and charts).

## **sysdigCommonConfig.skipStorageClass**

**Required**: `false`<br />
**Description**: When `true`, the installer does not apply the StorageClass manifest. Use this when you manage storage yourself (e.g. existing StorageClass or static provisioning). Setting it avoids unnecessary diff churn and deploy steps related to StorageClass, and aligns the installer with your cluster storage setup. The StorageClass used by the installer is selected via `.Values.global.storageClassName`.<br />
**Side effect**: The installer will not create a StorageClass. For new installations, a StorageClass (or equivalent) with the name specified in `.Values.global.storageClassName` must exist in the cluster before you run the installer, otherwise PVCs may remain Pending. Existing on-prem installations (where a suitable StorageClass is already in use) are not affected.<br />
**Options**: `true` \| `false`<br />
**Default (chart value)**: `false` (installer deploys the StorageClass).<br />
**Default (installer)**: `true` (installer does not deploy the StorageClass unless you override this value).<br />
**Example**:
