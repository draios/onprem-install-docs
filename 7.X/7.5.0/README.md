...
2. Pull the image containing the self-extracting tar:
  ```bash
  docker pull quay.io/sysdig/installer:7.5.0-1-uber
  ```
3. Extract the tarball:
  ```bash
  docker create --name uber_image quay.io/sysdig/installer:7.5.0-1-uber
...
**Note:** Values shown for Secure Only mode and Platform mode assume that `scanningv2`, `kspm`, and `graph` are enabled.
