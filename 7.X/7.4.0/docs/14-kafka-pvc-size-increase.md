...
## Increase Kafka Storage when disk is full

This runbook describes the process to increase persistent storage for Kafka statefulset when the disk gets full.
...
Increasing the storage size in this case requires statefulset patching beforehand. Also, we need to check for:
...
~> kubectl delete pvc data-cp-kafka-0 -n sysdigcloud &
persistentvolumeclaim "data-cp-kafka-0" deleted
~> kubectl delete pv pvc-95413869-b940-4435-affc-81de2bc30b5f &
...
~> kubectl get pvc -n sysdigcloud | grep data-cp-kafka
NAME                            STATUS   VOLUME                      CAPACITY   ACCESS MODES   STORAGECLASS   AGE
data-cp-kafka-0                 Bound    pvc-95413869-81de2bc30b5f   200Gi      RWO            gp2            58m
data-cp-kafka-1                 Bound    pvc-8d381527-fcb3689f7588   100Gi      RWO            gp2            58m
data-cp-kafka-2                 Bound    pvc-266e6b46-cb235a99b805   100Gi      RWO            gp2            58m
```
...
* Verify the following:
  ...
  * Statefulset spec shows volumeClaimTemplate storage size as updated size: `kubectl get statefulset cp-kafka -n sysdigcloud -o yaml`
  * All PVCs are `Bound` and showing up as the updated size: `kubectl get pvc -n sysdigcloud | grep data-cp-kafka`
