...
> [!WARNING]
> The following examples are only valid for versions >= 7.5.0
> For older versions, see the examples below.

#### Edit the following values (versions >= 7.5.0)

- `.global.size`
  - Specifies the size of the cluster. Size defines CPU, Memory, Disk, and Replicas. Valid options are:
    `small`, `medium` and `large`.

- `.global.quaypullsecret`
  - `quay.io` credentials provided with your Sysdig purchase confirmation mail.

- `.global.storageClassProvisioner`
  - The name of the storage class provisioner to use when creating the configured storageClassName parameter. If you do not use one of those two
  dynamic storage provisioners, then enter: hostPath and refer to the Advanced examples for how to configure static storage provisioning with this option. Valid options: `aws`, `gke`, `hostPath`.

- `.global.license`
  - Sysdig license key provided with your Sysdig purchase confirmation mail.

- `global.dnsName`
  - The domain name the Sysdig APIs will be served on.

- `.collector.dnsName`
  - (OpenShift installs only) Domain name the Sysdig collector will be served on. When not configured it defaults to whatever is configured for sysdig.dnsName.

- `.global.ingressNetworking`
  ...
  are:
    ...
    - `nodeport`: creates a service of type nodeport. The node ports can be
      customized with:
      - `.global.ingressNetworkingInsecureApiNodePort`
      - `.global.ingressNetworkingApiNodePort`
      - `global.ingressNetworkingCollectorNodePort`

**NOTE**: If doing an airgapped install (see airgapped Installation Options), you would also edit the following values:
...
- `global.airgappedRegistryPassword`:
  The password for the configured global.airgappedRegistryUsername. Ignore this
  parameter if the registry does not require authentication.

- `global.airgappedRegistryUsername`:
  The username for the configured global.airgappedRegistryName. Ignore this
  parameter if the registry does not require authentication.

> [!WARNING]
> The following examples are valid ONLY for version < 7.5.0
...
- [`global.airgappedRegistryName`](docs/configuration_parameters.md#global.airgappedRegistryName):
  The URL of the airgapped (internal) docker registry. This URL is used for
  installations where the Kubernetes cluster can not pull images directly from
  Quay.
- [`global.airgappedRegistryPassword`](docs/configuration_parameters.md#global.airgappedRegistryPassword):
  The password for the configured global.airgappedRegistryUsername. Ignore this
  parameter if the registry does not require authentication.
- [`global.airgappedRegistryUsername`](docs/configuration_parameters.md#global.airgappedRegistryUsername):
  The username for the configured global.airgappedRegistryName. Ignore this
  parameter if the registry does not require authentication.
...
### Step 4 (optional) - Execute a validation before deploying

Run the installer `deploy` in validation mode by adding the `--validate-only` flag. This runs all steps that lead to the deployment including a validation of the parameters but does not perform a deployment.
The validation consists of several checks on the parameters amongsts themselves and, in case of an upgrade, against the live objects in the cluster.

```shell
/home/user/installer-linux-amd64 deploy --validate-only
... [omitted lines]
I1110 20:47:53.471574   34260 deploy.go:246] validation stage:deploy passed
I1110 20:47:53.471611   34260 deploy.go:64] Validation passed - ready for deployment (no changes will be made to the cluster)
```

### Step 5 - Deploy Sysdig version
