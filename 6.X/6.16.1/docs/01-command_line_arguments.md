...
- If the pull secret is missing, the behaviour could be unpredictable:
  - Some Pods could start if they can find the image locally and if their `imagePullPolicy` is not `Always`. 
  is not `Always`
- Other Pods will fail because they cannot pull the image.

`--skip-serviceaccount`

- You must provide service accounts with the exact same name expected:
...
- You can use this flag to remove such a proxy configuration, if necessary.
